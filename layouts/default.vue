  ...
  <v-app dark>
    <v-app-bar app class="hidden-md-and-down" color="#272727">
      ...
      <!-- <v-toolbar-title v-if="!isLogged()">
        <NuxtLink class="header nolink" to="/login">
          Login
        </NuxtLink>
      </v-toolbar-title>
      <v-toolbar-title v-if="!isLogged()">
        <NuxtLink class="header nolink" to="/register">
          Register
        </NuxtLink>
      </v-toolbar-title>
      <v-toolbar-title v-if="isLogged()">
        <NuxtLink class="header nolink" to="/" @click="logout()">
          Logout
        </NuxtLink>
      </v-toolbar-title> -->
    </v-app-bar>
    ...
    <v-navigation-drawer v-model="drawer" class="hidden-md-and-up" absolute temporary>
      ...
      <v-list nav dense>
        <v-list-item-group v-model="group" active-class="deep-grey--text text--accent-4">
          ...
          <NuxtLink v-if="log" class="nolink" to="/register">
            <v-list-item class="mt-5">
              <v-list-item-icon>
                <v-icon>mdi-account</v-icon>
              </v-list-item-icon>
          ...
          <v-list-item v-if="!log" class="mt-5">
            <v-list-item-icon>
              <v-icon>mdi-account</v-icon>
            </v-list-item-icon>
            <v-list-item-title>
